...
void CRaytraceView::AddSomeSpheres()
{
	...
	auto shader = std::make_shared<CPhongLightShader>(yellow);
...
void CRaytraceView::AddSomeConicCylinders()
{
	...
	AddConicCylinder(std::make_shared<CPhongLightShader>(white), 2, 1, 1, transform);
	...
	AddConicCylinder(std::make_shared<CPhongLightShader>(red), 1, 1, 0, coneTransform);
	...
	AddConicCylinder(std::make_shared<CPhongLightShader>(green), 1, 0.5, 0.3, conicFrustumTransform);
}
...
void CRaytraceView::AddSomeTetrahedron()
{
	...
	AddTriangleMesh(std::make_shared<CPhongLightShader>(blue), std::make_shared<CTriangleMeshData>(std::move(vertices), faces), transform);
}
